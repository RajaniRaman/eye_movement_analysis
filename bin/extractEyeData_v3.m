...
for iexp = 1:length(br_dir) 
    ...
    for iday = 1:length(runs_dir) % iteration over days
        ...
        json_dile = fileread(fullfile(calibration_path, 'eyecalibration.json'));
        cal_data = jsondecode(json_dile);
        calPoints = cal_data.points;
        calPoints_centrd = calPoints - calPoints(1, :);
        
        sclFactor = 1./cal_data.toVisualDegrees; % using avalible scaling factor
        sclFactor = sclFactor';
        
        
%         % scaling factor to convert voltage to degree
%         mean_calPoints = mean(abs(calPoints_centrd)) * 2;
%         horzScale = mean_calPoints(1) / hDeg; % voltage per degree
%         vertScale = mean_calPoints(2) / vDeg; % voltage per degree
%         sclFactor = [horzScale, vertScale];
...
%         % creating a expt directory and saving the data in a .csv file
%         output_dir = sprintf('../results/data/%s', expname);
